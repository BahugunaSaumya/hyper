...
function getProductByQuery(q) {
    ...
    // slug match
    const qs = t;
    return products.find(x => x.title === qs) || null;
}
...
async function renderProductDetail(product) {
  ...
  `;
if (window.__DISABLE_LEGACY_ROUTER__) {
  window.navigate = (url) => { location.href = url; };
  window.routeTo  = (path, search) => { location.href = path + (search || ""); };
  // Skip binding click/popstate handlers here.
}
...
// --- gallery discovery: CSV image + ./assets/models/products/<slug>/<n>.ext ---
async function discoverGalleryImages(mainImagePath) {
  const images = [];

  // 1) Always include the CSV image (force absolute /assets/…)
  const absCsv = mainImagePath.replace(/^\.\//, '/'); // './assets/…' -> '/assets/…'
  images.push(absCsv);

  // 2) Figure out the "nameOnly" from CSV basename (used as folder name)
  try {
    const csvUrl = new URL(absCsv, location.origin);
    const parts = csvUrl.pathname.split('/'); // /assets/models/<file>
    const file = parts.pop() || '';
    const baseDir = parts.slice(0, -1).join('/') + '/'; // /assets/models/
    const nameOnly = file.replace(/\.[^.]+$/, '');      // <slug> (no extension)

    // 3) Candidate gallery folder: /assets/models/products/<slug>/
    const folder = baseDir + 'products/' + nameOnly + '/';

    // Your naming: "1 - Main", "1", "01", "main", "Main", "2", "3", "4", "5", "6"
    const baseNames = ['1 - Main', '1', '01', 'main', 'Main', '2', '3', '4', '5', '6'];
    const exts = ['jpg'];
    ...
    // 4) Probe candidates; keep the ones that actually exist
    const found = await probeImages([...new Set(candidates)]);

    for (const src of found) {
      if (!images.includes(src)) images.push(src);
    }
  } catch (e) {
    console.warn('[gallery] cannot parse CSV image path', mainImagePath, e);
  }

  return images;
}

async function probeImages(urls, timeoutMs = 3500) {
...
if (document.readyState !== 'loading') {
  try { navigate(location.pathname + location.search, { push: false }); }
  catch (e) { console.error('[late boot] navigate failed', e); }
}